...
final class IndexedMaterializedStoreReader<T extends SpillableObject>
	...
	public OOCFuture<MaterializedStore.Lease<T>> request(int index) {
		...
		StorePinRetry.pinWithRetry(_cache, _streamId, index, _allowance, () -> _closed, pinned);
	...
	public MaterializedStore.Lease<T> requestIfLive(int index) {
		checkReady(index);
		reserve(index);
		BlockEntry entry = _cache.pinIfLive(_streamId, index, _allowance);
		if(entry == null) {
			_liveness.unreserve(index);
			return null;
		}
