...
public class OOCPlanner {
	...
	private static void findLeaves(OOCPrimitive primitive, Set<OOCPrimitive> leaveList) {
		if(primitive.isLeaf()) {
			leaveList.add(primitive);
			return;
		}

		for(OOCPrimitive child : primitive.getChildren()) {
			findLeaves(child, leaveList);
		}
	}

	private static void inferAccessPatterns(Set<OOCPrimitive> leaves) {
		for(OOCPrimitive child : leaves) {
			child.inferPatterns();
		}
	}
}
