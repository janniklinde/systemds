...
public final class GeneralMMultOOCPrimitive extends OOCPrimitive {
	...
	protected void startExecution() {
		DataCharacteristics left = _left.getDataCharacteristics();
		DataCharacteristics right = _right.getDataCharacteristics();
		_rowBlocks = Math.toIntExact(left.getNumRowBlocks());
		_innerBlocks = Math.toIntExact(left.getNumColBlocks());
		_colBlocks = Math.toIntExact(right.getNumColBlocks());
	...
	private void storesReady(List<MaterializedStore<IndexedMatrixValue>> stores, Throwable error) {
		...
		OOCFuture.allOf(List.of(_leftStore.completion(), _rightStore.completion()), ignored -> {
	...
	private static final class MultiplyWork implements ComputeWork {
