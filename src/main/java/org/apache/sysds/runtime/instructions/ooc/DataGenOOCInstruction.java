		...
		if (method == Types.OpOpDG.RAND) {
			...
			if(OOC_NEW_SYSTEM) {
				...
				primitive.setProducer(alloc -> {
					int ctr = 0;
					int k = mK.getValue();
					double curFrom = mCurFrom.getValue();
					double curTo;
					MatrixBlock mb;
					...
					while(++ctr <= Math.max(1, alloc / (Math.max(1, blen) * 8L)) && k < maxK) {
						...
						primitive.emit(new IndexedMatrixValue(new MatrixIndexes(1 + k / blen, 1), mb));
