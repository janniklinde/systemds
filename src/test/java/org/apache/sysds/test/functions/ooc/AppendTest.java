...
public class AppendTest extends AutomatedTestBase {
	private static final String TEST_NAME = "Append";
	private static final String TEST_DIR = "functions/ooc/";
	private static final String TEST_CLASS_DIR = TEST_DIR + AppendTest.class.getSimpleName() + "/";
	private static final int ROWS = 300;
	private static final int LEFT_COLS = 600;
	private static final int RIGHT_COLS = 500;
	...
	@Test
	public void testAppendWithSpillLimit() {
		Types.ExecMode platform = setExecMode(Types.ExecMode.SINGLE_NODE);
		try {
			getAndLoadTestConfiguration(TEST_NAME);
			fullDMLScriptName = SCRIPT_DIR + TEST_DIR + TEST_NAME + ".dml";
			MatrixBlock left = MatrixBlock.randOperations(ROWS, LEFT_COLS, 0.2, -1, 1, "uniform", 7);
			MatrixBlock right = MatrixBlock.randOperations(ROWS, RIGHT_COLS, 0.2, -1, 1, "uniform", 8);
			writeInput(left, "A");
			writeInput(right, "B");
			...
			Assert.assertTrue(heavyHittersContainsString(Instruction.OOC_INST_PREFIX + Opcodes.APPEND));
			OOCCacheManager.reset();

			programArgs = new String[] {"-stats", "-args", input("A"), input("B"), output("expected")};
			runTest(true, false, null, -1);
			MatrixBlock actual = DataConverter.readMatrixFromHDFS(output("result"), Types.FileFormat.BINARY, ROWS,
				LEFT_COLS + RIGHT_COLS, BLOCKSIZE);
