...
public class AggregateTernaryTest extends AutomatedTestBase {
	private static final String TEST_NAME = "AggregateTernary";
	private static final String TEST_DIR = "functions/ooc/";
	private static final String TEST_CLASS_DIR = TEST_DIR + AggregateTernaryTest.class.getSimpleName() + "/";
	...
	@Test
	public void testAggregateTernary() throws IOException {
		...
		try {
			...
			for(int operation = 1; operation <= 4; operation++) {
				programArgs = arguments(true, operation, "ooc");
				runTest(true, false, null, -1);
				String opcode = operation % 2 == 1 ? Opcodes.TAKPM.toString() : Opcodes.TACKPM.toString();
				Assert.assertTrue(heavyHittersContainsString(Instruction.OOC_INST_PREFIX + opcode));

				programArgs = arguments(false, operation, "target");
				runTest(true, false, null, -1);
				int columns = operation % 2 == 1 ? 1 : COLS;
				MatrixBlock actual = DataConverter.readMatrixFromHDFS(output("ooc"), Types.FileFormat.BINARY, 1,
					columns, BLOCK_SIZE);
				MatrixBlock expected = DataConverter.readMatrixFromHDFS(output("target"), Types.FileFormat.BINARY, 1,
					columns, BLOCK_SIZE);
